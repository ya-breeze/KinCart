...
- The list detail lets the shopper change the list's shop while shopping; the change is saved to the list.
...
- A shop change made from the list detail persists back onto the list.
...
- **Frontend:** `Dashboard.jsx` create-list dialog gains a shop `<select>` and sends `shop_id`. `ListDetail.jsx` derives the selected shop from `list.shop_id`, and `handleShopChange` PATCHes the list. The selector lives in the shopper view; the manager sets the shop at creation.
