...
- [AI latency/cost] → Only call Gemini when history misses, and only on the two paths that can absorb it: batched into the existing parse call for paste, and inside the background receipt ticker (guarded by `s.gemini != nil`). The synchronous add paths (`AddItemToList`/`BulkAddItems`) never call Gemini, so a manual add is no slower than today.
