...
- [ ] 1.5 Handler tests for both transitions: marking an absent item bought clears `is_absent`; marking a bought item absent returns 400 and leaves the row unchanged
...
- [ ] 2.2 Add a "not available" control next to the check-off on active shopper items; do not render it on bought items (the backend rejects that transition)
...
- [ ] 4.1 Render a "not found" badge on absent items in the manager view, in place within their category group (vanilla CSS per `.cursorrules`)
