...
- [Two booleans can still represent "bought AND absent" in the database] → The type does not prevent it; the `UpdateItem` guard and the receipt-service clear do. Every write path to `is_bought`/`is_absent` runs through one of those two, so the invariant holds — but a future third write path would have to honour it deliberately. This is the price of not taking the enum, and the reason the guard lives in the handler rather than the frontend.
