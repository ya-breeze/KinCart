# Feature: Frequent Items & Item Aliases

## Requirement: Frequency tracking
Every time an item is added to a list its frequency count is incremented.
...
The manager can remove an item from the suggestion chips without deleting its history.
...
An alias maps a generic planned name (e.g., "Milk") to a specific receipt name (e.g., "Parmalat UHT 1L") at a given shop.
...
The manager can view and edit all aliases from the Settings or Aliases page.
...
Typing in the quick-add bar triggers autocomplete from alias history.
