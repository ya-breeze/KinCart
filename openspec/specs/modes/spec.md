# Manager & Shopper Modes

## Purpose
Provide Manager and Shopper modes that tailor available actions and list visibility.

## Requirements

### Requirement: Mode switching

Every user SHALL be able to toggle between Manager mode and Shopper mode at any time.
...
Manager mode SHALL provide full planning and administrative access.
...
Shopper mode SHALL provide a focused, read-only view optimized for in-store use.
...
Mode SHALL NOT affect server-side authorization; the same API endpoints serve both modes.
