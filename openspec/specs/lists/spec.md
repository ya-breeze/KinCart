...
A user can create a new shopping list with a title.
...
A list moves through three statuses: `preparing` → `ready for shopping` → `completed`.
...
Lists are grouped and ordered by status on the Dashboard.
...
Each list card on the Dashboard shows a summary of the list's state.
...
A manager can duplicate an existing list to reuse it.
...
A manager can delete a list at any status.
